...
### Discriminated generic overloading example
...
function Profile() {
  // Working context
  const { status, user } = useAuthContext('authenticated');
  const { status, user } = useAuthContext('default'); // user might be undefined

  // Not working contexts
  // const { status, user } = useAuthContext(); // not allowed empty ()
  // const { status, user } = useAuthContext('error'); // no user
  // const { status, user } = useAuthContext('idle'); // no user
